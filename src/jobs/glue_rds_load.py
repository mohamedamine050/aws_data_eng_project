...
# The schema ``sql/warehouse/001_schema.sql`` creates and every table lives in.
...
def _resolve_rds_settings(config: dict) -> dict:
    ...
    settings = {
        ...
        "schema": config.get("RDS_SCHEMA", DEFAULT_SCHEMA),
...
#
# Spark's JDBC writer creates a *table*, never the schema holding it. Writing
# ``analytics.fact_events`` into a database where nobody ran the warehouse DDL
# fails with ``schema "analytics" does not exist`` — on the first target, after
# the cluster has spun up and read the data. Creating it here costs one
# statement and removes that failure mode from every fresh environment.
# ─────────────────────────────────────────────

def _validate_identifier(identifier: str) -> None:
    """``CREATE SCHEMA`` takes no placeholder, so the name is concatenated.

    It comes from the config file, which is not a trusted input in the way a
    literal is. Anything that is not a bare identifier is refused before it can
    reach the database.
    """
    if not identifier:
    ...
    if not all(char in allowed for char in identifier):
        raise ValueError(f"Invalid PostgreSQL identifier: {identifier}")


def _ensure_schema(spark: "SparkSession", settings: dict) -> None:
    """``CREATE SCHEMA IF NOT EXISTS`` over Spark's JVM gateway.

    A plain JDBC connection, because Spark's DataFrame API cannot issue DDL of
    its own. Quoted so the name survives PostgreSQL's lower-casing, and
    autocommit so the statement is durable before the writers open their own
    connections.
    """
    schema = settings.get("schema")
    if not schema:
        logger.warning("No RDS_SCHEMA configured: the tables follow the search_path.")
        return
    ...
    statement = None
    try:
        logger.info("Creating schema %s if it does not exist", schema)
        driver_manager = spark.sparkContext._gateway.jvm.java.sql.DriverManager
        connection = driver_manager.getConnection(
            jdbc_url, settings["username"], settings["password"]
        )
        connection.setAutoCommit(True)
        statement = connection.createStatement()
        statement.executeUpdate(f'CREATE SCHEMA IF NOT EXISTS "{schema}"')
        logger.info("Schema %s is ready", schema)
    except Exception as exc:
        # Carrying on means writing into a schema that is not there: the load
        # fails anyway, several minutes later and with a worse message.
        logger.error("Could not create schema %s: %s", schema, exc)
        raise
    finally:
        # RDS caps connections; a leak per run eventually locks the job out.
        for resource in (statement, connection):
...
def main() -> None:
    ...
    _ensure_schema(spark, rds_settings)
        ...
        json.dumps(
            {
                "status": "success",
                "mode": args["mode"],
                "schema": rds_settings.get("schema"),
                "targets": results,
