"""The Glue scripts are standalone — this file is what keeps that safe.

Each job carries its own copy of the lake layout, so nothing has to be shipped
alongside its script and ``--extra-py-files`` stays empty. The cost of that
choice is four copies that could drift, and a drift is not a loud failure: the
landing job would keep writing to ``bronze/events/`` while the silver job
quietly read somewhere else, and a day of data would simply look missing.

So the copies are pinned here against ``common/lakehouse.py`` — the definition
the Lambdas use. If one of them stops agreeing, the build fails instead of the
pipeline.
"""
...
import jobs.glue_bronze_to_silver as job_silver
...
JOBS = {
    ...
    "glue_bronze_to_silver": job_silver,
    "glue_silver_to_gold": job_gold,
    "glue_rds_load": job_load,
}

#: Configs worth checking: the medallion defaults, a relocated layout, a
#: pre-medallion deployment still on the old keys, and an explicit override.
CONFIGS = [
    {"OUTPUT_BUCKET": "lake"},
...
def test_a_job_script_imports_nothing_from_this_repository(name):
    """What a Glue job's Script path points at is all it runs.

    An import of ``common`` or of a sibling job would mean the script has to be
    packaged with something, and `--extra-py-files` is empty by design.
    """
    tree = ast.parse(Path(JOBS[name].__file__).read_text(encoding="utf-8"))
    ...
    assert "spark" not in imported, f"{name} imports a shared Spark module"
...
def test_dataset_paths_match_the_lambda_definition(name, config):
    """The one that actually matters: where the landing job writes is where the
    silver job must read."""
    job = JOBS[name]
...
def test_metrics_carry_the_stage_and_environment(name):
    ...
    assert namespace == "NS"
    assert {d["Name"]: d["Value"] for d in data[0]["Dimensions"]} == {
        "Stage": "a_stage", "Environment": "prod",
    }
